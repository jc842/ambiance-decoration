# Directives & Contraintes d'Environnement — ambiance-decoration.com

Ce document consigne l'architecture technique, les contraintes d'environnement, les directives de sécurité et les règles éditoriales pour le site **ambiance-decoration.com** (ambiance-decoration) hébergé sur Cloudflare Pages.
...
| :--- | :--- |
| **Domaine Apex** | `ambiance-decoration.com` |
| **URL Cloudflare Pages** | `https://ambiance-decoration.pages.dev` |
| **Dépôt GitHub** | `jc842/ambiance-decoration` (branche principale : `main`) |
...
| **Matomo Site ID** | `79` |
| **Matomo MTM Container** | `container_RXdkYyk6.js` (Container ID : `RXdkYyk6`) |
| **Serveur Publicitaire** | Revive Adserver (`https://ads.les4h.fr/`) |
| **Revive Affiliate ID** | `99` |
| **Zones Publicitaires** | Header (`608`), In-Content (`607`), Mobile Sticky (`605`), Large Rectangle (`606`), Half Page (`609`), Skyscraper (`604`) |
...
- Ce Layout injecte au build statique le composant `Matomo.astro` avec le conteneur MTM dédié : **`container_RXdkYyk6.js`**.
...
- Tout article doit obligatoirement comporter les 2 emplacements publicitaires intégrés dans le gabarit (`src/pages/blog/[...slug].astro`) :
  1. **Encart In-Content (300x250)** : `<ReviveSlot slot="inContent" />` positionné après les premiers paragraphes (Zone 607).
  2. **Encart Fin d'Article (336x280)** : `<ReviveSlot slot="largeRectangle" />` positionné avant les suggestions de maillage (Zone 606).
